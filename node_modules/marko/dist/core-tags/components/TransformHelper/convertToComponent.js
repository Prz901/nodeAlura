...
module.exports = function handleComponentBind(options) {
    ...
    if (rendererModule && rendererModule !== componentModule || !rendererModule && componentModule) {
        if (hasTagParams) {
            context.addError(context.exampleTagParam, "Cannot use tag params within a split component.");
        } else {
            componentProps.d_ = isSplit = true;
        }
    }
    ...
    if (componentModule) {
        ...
        componentProps.f_ = componentTypeNode;

        context.setMeta("id", componentType.id);

        if (isLegacyComponent) {
            let el = rootNodes[0];
            el.setAttributeValue("data-widget", builder.literal(componentType.id));
            if (!el.hasAttribute("id")) {
                el.setAttributeValue("id", this.buildComponentElIdFunctionCall());
            }
        }
        ...
        if (isSplit) {
            context.addDependency(context.markoModulePrefix + "runtime/components");
        }
    }

    if (isLegacyInnerBind) {
        let el = rootNodes[0];
        el.setAttributeValue("id", builder.memberExpression(builder.identifier("__component"), builder.identifier("id")));
    ...
    var rendererHelper = isLegacyComponent ? this.context.helper("rendererLegacy") : this.context.helper("renderer");
    ...
    this.context.on("beforeGenerateCode:TemplateRoot", function (eventArgs) {
        ...
        eventArgs.node.generateAssignRenderCode = eventArgs => {
            ...
            const createRendererArgs = [renderFunctionVar, builder.literal(componentProps)];

            if (markoComponentVar) {
                createRendererArgs.push(markoComponentVar);
            }

            nodes.push(builder.assignment(templateRendererMember, builder.functionCall(rendererHelper, createRendererArgs)));

            if (!isSplit && !isLegacyComponent) {
                nodes.push(builder.assignment(builder.memberExpression(templateVar, builder.identifier("Component")), builder.functionCall(defineComponentHelper, [markoComponentVar || builder.literal({}), templateRendererMember])));
            }
