...
class TransformHelper {
    ...
    hasBoundComponentForTemplate() {
        return this.context.data.componentModule != null || this.context.data[HAS_COMPONENT_KEY] || this.context.data[WIDGET_PROPS_KEY] != null;
    }
    ...
    serializeKey() {
        ...
        if (!this.__keySerialized && context.isServerTarget() && context.isSplitComponent) {
            var markoKeyAttrVar = context.importModule("marko_keyAttr", this.getMarkoComponentsRequirePath("marko/core-tags/components/helpers/markoKeyAttr"));

            el.setAttributeValue("data-marko-key", builder.functionCall(markoKeyAttrVar, [key, builder.identifier("__component")]));
    ...
    get markoComponentsVar() {
        if (!this.context.data[MARKO_WIDGETS_VAR_KEY]) {
            this.context.data[MARKO_WIDGETS_VAR_KEY] = this.context.importModule("marko_components", this.getMarkoComponentsRequirePath(this.isLegacyComponent ? "marko/components/legacy" : "marko/components"));
        }
    ...
    buildComponentElIdFunctionCall(id) {
        ...
        if (id && id.type === "Literal" && id.value === "") {
            let componentElId = builder.memberExpression(builder.identifier("__component"), builder.identifier("id"));

            return componentElId;
        } else {
            let componentElId = builder.memberExpression(builder.identifier("__component"), builder.identifier("elId"));
