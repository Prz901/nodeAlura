...
module.exports = function migrate(el, context) {
    ...
    const renderBodyValue = builder.memberExpression(bodyValue, builder.identifier("renderBody"));
    ...
    el.appendChildren([builder.htmlElement("if", undefined, [dynamicTag], printJS(isDefault ? condition : builder.binaryExpression(bodyValue, "&&", builder.binaryExpression(buildTypeOfFunction(bodyValue, context), "||", condition)), context)), builder.htmlElement("else", undefined, [builder.text(isDefault ? renderBodyValue : bodyValue)])]);

    if (el.hasAttribute("body-only-if") && el.getAttribute("body-only-if").argument === "true") {
...
function buildTypeOfFunction(node, context) {
    const builder = context.builder;
    return builder.binaryExpression(builder.unaryExpression(node, "typeof", true), "===", builder.literal("function"));
}