...
var VElement = require("../vdom").aR_;
var virtualizeElement = VElement.bH_;
var morphAttrs = VElement.bI_;
...
var beginFragmentNode = fragment.bO_;
...
function morphdom(fromNode, toNode, doc, componentsContext) {
    ...
    if (componentsContext) {
        ...
        isRerenderInBrowser = globalComponentsContext._r_;
    }

    function insertVirtualNodeBefore(vNode, key, referenceEl, parentEl, ownerComponent, parentComponent) {
        var realNode = vNode.bq_(doc, parentEl.namespaceURI);
        insertBefore(realNode, referenceEl, parentEl);

        if (vNode.bt_ === ELEMENT_NODE || vNode.bt_ === FRAGMENT_NODE) {
            if (key) {
    ...
    function morphChildren(fromNode, toNode, parentComponent) {
        ...
        outer: while (curToNodeChild) {
            ...
            var ownerComponent = curToNodeChild.bk_ || parentComponent;
            ...
            if (curToNodeType === COMPONENT_NODE) {
                ...
                if (curFromNodeKey === curToNodeKey) {
                    ...
                    if ((curToNodeChild.g_ & FLAG_PRESERVE) === 0 && !curToNodeChild.bv_) {
                    ...
                    if ((matchingFromEl = referenceComponent.m_[curToNodeKey]) === undefined) {
                        if (isRerenderInBrowser === true && curFromNodeChild) {
                            if (curFromNodeChild.nodeType === ELEMENT_NODE && caseInsensitiveCompare(curFromNodeChild.nodeName, curToNodeChild.bB_ || "")) {
                                curVFromNodeChild = virtualizeElement(curFromNodeChild);
                                curVFromNodeChild.bB_ = curToNodeChild.bB_;
                                keysByDOMNode.set(curFromNodeChild, curToNodeKey);
                                morphEl(curFromNodeChild, curVFromNodeChild, curToNodeChild, curToNodeKey, ownerComponent, parentComponent);
                                curToNodeChild = toNextSibling;
                                curFromNodeChild = fromNextSibling;
                                continue;
                            } else if (curToNodeChild.bt_ === FRAGMENT_NODE && curFromNodeChild.nodeType === COMMENT_NODE) {
                        ...
                        if ((curToNodeChild.g_ & FLAG_PRESERVE) === 0 && !curToNodeChild.bv_) {
                            ...
                            if (compareNodeNames(curVFromNodeChild, curToNodeChild)) {
                                if (fromNextSibling === matchingFromEl) {
                                    ...
                                    if (toNextSibling && toNextSibling.bu_ === curFromNodeKey) {
                                        // Single element swap
                    ...
                    if (curFromNodeType === ELEMENT_NODE) {
                        ...
                        if (curVFromNodeChild === undefined) {
                            if (isRerenderInBrowser === true) {
                                ...
                                if (caseInsensitiveCompare(curVFromNodeChild.bB_, curToNodeChild.bB_)) {
                                    curVFromNodeChild.bB_ = curToNodeChild.bB_;
                                }
                            } else {
        ...
        if (fromNode.bN_) {
            ...
            while (curFromNodeChild) {
                ...
                if (isAutoKey(keysByDOMNode.get(fromNode))) {
                    ...
                    referenceComponent = curVFromNodeChild && curVFromNodeChild.bk_;
                }
    ...
    function morphEl(fromEl, vFromEl, toEl, toElKey, ownerComponent, parentComponent) {
        var nodeName = toEl.bB_;

        if (isRerenderInBrowser === true && toElKey) {
            ownerComponent.m_[toElKey] = fromEl;
        }

        var constId = toEl.bD_;
        if (constId !== undefined && vFromEl.bD_ === constId) {
            return;
        }
