"use strict";

const ComponentDef = require("./ComponentDef");

var FLAG_WILL_RERENDER_IN_BROWSER = 1;
// var FLAG_HAS_BODY_EL = 2;
// var FLAG_HAS_HEAD_EL = 4;
var FLAG_OLD_HYDRATE_NO_CREATE = 8;

module.exports = function beginComponent(componentsContext, component, key, ownerComponentDef, isSplitComponent, isImplicitComponent) {
    var globalContext = componentsContext.l_;

    var componentId = component.id;

    var componentDef = componentsContext.j_ = new ComponentDef(component, componentId, globalContext);

    // On the server
    if (ownerComponentDef && ownerComponentDef.g_ & FLAG_WILL_RERENDER_IN_BROWSER) {
        componentDef.g_ |= FLAG_WILL_RERENDER_IN_BROWSER;
        return componentDef;
    }

    if (isImplicitComponent === true) {
        // We don't mount implicit components rendered on the server
        // unless the implicit component is nested within a UI component
        // that will re-render in the browser
        return componentDef;
    }

    componentsContext.h_.push(componentDef);

    let out = componentsContext.z_;
    let runtimeId = out.global.runtimeId = out.global.runtimeId || "M";

    componentDef._D_ = true;

    if (isSplitComponent === false && out.global.noBrowserRerender !== true) {
        componentDef.g_ |= FLAG_WILL_RERENDER_IN_BROWSER;
    }

    if (out.global.oldHydrateNoCreate === true) {
        componentDef.g_ |= FLAG_OLD_HYDRATE_NO_CREATE;
    }

    if (ownerComponentDef && key != null) {
        out.w("<!--" + runtimeId + "^" + componentId + " " + ownerComponentDef.id + " " + key + "-->");
    } else {
        out.w("<!--" + runtimeId + "#" + componentId + "-->");
    }

    return componentDef;
};