...
function getRootDeps(template, context) {
    if (production && template.as_) {
        return template.as_;
    }

    attachDepsAndComponentsToTemplate(template, context);

    var deps = template.as_ = Object.keys(template.at_).map(key => template.at_[key]);
...
function attachDepsAndComponentsToTemplate(target, context) {
    ...
    if (production && target.at_) return;
    ...
    if (meta.tags) {
        meta.tags.forEach(tagPath => {
            ...
            if (tag.at_) Object.assign(deps, tag.at_);
